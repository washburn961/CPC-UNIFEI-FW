...
typedef struct {
    ...
    complex_t current[PHASE_COUNT]; /**< Current measurements for phases A, B, and C */
    float pick_up_current; /**< Pickup current setting */
    float time_step; /**< Simulation time step */
    float time[PHASE_COUNT]; /**< Accumulated time for each phase */
    float time_to_trip[4]; /**< Time to trip for each phase */
    unsigned int is_pickupped[PHASE_COUNT]; /**< Pick up status flags for each phase */
    unsigned int is_tripped[PHASE_COUNT]; /**< Trip status flags for each phase */
